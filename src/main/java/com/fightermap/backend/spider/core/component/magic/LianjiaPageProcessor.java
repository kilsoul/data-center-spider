...
public class LianjiaPageProcessor implements PageProcessor {
    ...
    public void process(Page page) {
        ...
        if (PageUtil.isMatch(url, Regex.Lianjia.HOME)) {
        ...
        } else if (PageUtil.isMatch(url, Regex.Lianjia.REGION)) {
            ...
            page.putField(ITEM_KEY_HOUSE_SHORT_INFO_LIST, houseShortInfoList.stream().filter(houseShortInfo -> positionRequests.contains(houseShortInfo.getDetailUrl())).collect(Collectors.toList()));
            //低优先级
            page.addTargetRequests(houseShortInfoRequests, SpiderLevel.LOW.getValue());
